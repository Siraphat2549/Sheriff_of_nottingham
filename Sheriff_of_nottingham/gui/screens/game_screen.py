...
class GameScreen:
    ...
    def _toggle_discard(self, card: Card):
        ...
        if over <= 0:
            return
        if card in self._hand_over_limit:
            self._hand_over_limit.remove(card)
        else:
    ...
    def _draw_center_market(self, surface):
        ...
        for i, card in enumerate(e.market_cards):
            ...
            # Clickable only if player hasn't drawn yet and no privacy
            if not self._privacy_player and self._market_drew == 0:
                card_ref = card
    ...
    def _draw_action_market(self, surface, merchant: Player, az_y: int):
        ...
        for i, card in enumerate(hand):
            r = pygame.Rect(sx + i*(cw+gap), sy, cw, ch)
            is_discard = card in self._hand_over_limit
